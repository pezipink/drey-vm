  ...
  while(true)
    {    
      if(!receiveTimeout
              ...
              case MessageType.Debug:
                {
                  try
                    {
                      switch(processDebugMessage(vm,&message)) 
                        {
                        case DebugResponseAction.Send:
                          send(parentId, message);
                          break;
        ...
        {
          if(MonoTime.currTime - vm.lastHeart > dur!"msecs"(1000))
            {
              vm.lastHeart = MonoTime.currTime;
